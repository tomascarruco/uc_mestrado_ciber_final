...
// AES192 globals - note the change from AES128 to AES192
AES192 aes192;

// AES192 requires a 24-byte key (192 bits) instead of 16 bytes
...
encryptDataAES (const byte *data, int data_size, byte *out)
{
    ...
    for (int i = 0; i < block_count; ++i) {
        ...
        if (remaining >= AES_BLOCK_SIZE) {
            // Full block - encrypt directly using AES192
